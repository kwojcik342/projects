...
def upd_income():
    if not flask.request.json or "id_income" not in flask.request.json or "inc_amount" not in flask.request.json or "inc_tmst" not in flask.request.json:
        flask.abort(400)
    res = dbo.update_income(flask.request.json["id_income"]
                                  , flask.request.json["inc_amount"]
                                  , flask.request.json["inc_tmst"]
                                  , flask.request.json.get("inc_note", ""))
...
@app.route("/spendingstracker/api/incomes", methods=["DELETE"])
def del_incomes():
    if not flask.request.json or "incomes" not in flask.request.json:
        flask.abort(400)
    res = dbo.delete_incomes(flask.request.json["incomes"])
    return flask.jsonify({"result": res})
...
def upd_spending():
    if not flask.request.json or "id_spending" not in flask.request.json or "sp_amount" not in flask.request.json or "sp_tmst" not in flask.request.json:
        flask.abort(400)
    res = dbo.update_spending(flask.request.json["id_spending"]
                                  , flask.request.json["sp_amount"]
                                  , flask.request.json["sp_tmst"]
                                  , flask.request.json.get("sp_note", ""))
    return flask.jsonify({"result": res})
...
def get_user_spendings():
    ...
    sp_list = dbo.get_user_spendings(flask.request.json["id_user"]
                                     , flask.request.json.get("start_date", "")
                                     , flask.request.json.get("end_date", ""))
    return flask.jsonify({"spendings": sp_list})


@app.route("/spendingstracker/api/spendings", methods=["DELETE"])
def del_spendings():
    if not flask.request.json or "spendings" not in flask.request.json:
        flask.abort(400)
    res = dbo.delete_spendings(flask.request.json["spendings"])
    return flask.jsonify({"result": res})
